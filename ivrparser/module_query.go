	...
	for immersion > 0 {
		...
		case xml.StartElement:
			...
			if v.Name.Local == "fetchTimeout" {
				...
				pQM.RequestInfo.parseRequestInfo(decoder)
				immersion--
				...
				pQM.parseGeneralInfo(decoder, &v)
			}
...
///-------------
func (p *requestInfo) parseRequestInfo(decoder *xml.Decoder) error {
	var (
		immersion      = 1
		tmp            *replacement
		inTemplate     = false
...
/////////////
func parseResponseInfo(decoder *xml.Decoder) (p *responseInfo, err error) {
	var (
		immersion = 1
	)
	p = new(responseInfo)
	...
	for immersion > 0 {
		...
		case xml.StartElement:
			...
			if v.Name.Local == "from" {
				...
				p.parseRegexpInfo(decoder)
				immersion--
			} else if v.Name.Local == "func" {
				p.parseFunctionInfo(decoder)
				immersion--
			}
...
//---->
func (p *responseInfo) parseFunctionInfo(decoder *xml.Decoder) error {
	var (
		immersion    = 1
		inReturnType = false
		inName       = false
...
//+++++++++
func (p *responseInfo) parseRegexpInfo(decoder *xml.Decoder) error {
	var (
		immersion                        = 1
...
/////////////
func parseKeyValueList(decoder *xml.Decoder) (p []keyValue, err error) {
