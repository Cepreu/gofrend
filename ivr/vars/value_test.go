...
func TestString(t *testing.T) {
	...
	for _, turple := range stringArray {
		sval := NewString(turple.s, 0)

		if len(sval.String()) != turple.v {
			t.Errorf("TestString: len(%s). Expected %d, got %d", turple.s, turple.v, len(turple.s))
		}
	}
}
